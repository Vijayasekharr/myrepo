...
public class Ticket_ServicesImpl implements Ticket_ServicesI {
	...
	public FinalTicketResponse bookTicket(TicketRequest ticketRequest) {
		...
		FinalTicketResponse ticket = WebClient.create().post().uri(URL).headers(headers -> headers.setBasicAuth("yatra", "yatra123"))
				.bodyValue(ticketRequest).retrieve().bodyToMono(FinalTicketResponse.class).block();
	...
	public FinalTicketResponse updateTicket(FinalTicketResponse ticket) {
		ticket.setProvider("IRCTC");

//		/* Connecting to IRCTC-Application through WebClient */
		String URL = "http://localhost:2000/sbms/irctc/updateTicket";
...
//		/* Connecting to IRCTC-Application through RestTemplate */
		RestTemplate restTemplate = new RestTemplate();

		HttpHeaders httpHeaders = new HttpHeaders();
		httpHeaders.setBasicAuth("yatra", "yatra123");
//		httpHeaders.setContentType(MediaType.APPLICATION_JSON);
		HttpEntity<FinalTicketResponse> httpEntity = new HttpEntity<FinalTicketResponse>(ticket, httpHeaders);

		ResponseEntity<FinalTicketResponse> exchange = restTemplate.exchange(URL, HttpMethod.PUT, httpEntity, FinalTicketResponse.class);
	...
	@Override
	public StationsList getAvailable_Stations_between_Two_Stations(String from_station, String to_station, String quota,Integer train_no) {
		
		/* Connecting to IRCTC-Application through WebClient */
		String URL = "swapna:2000/sbms/train/getAvailable_Stations_between_Two_Stations?from_station="+from_station+"&to_station="+to_station+"&quota="+quota+"&train_no="+train_no;
		StationsList stationsList = WebClient.create()
				.get()
				.uri(URL)
//				.headers(headers -> headers.setBasicAuth("yatra", "yatra123"))
				.retrieve()
				.bodyToMono(StationsList.class)
				.block();
